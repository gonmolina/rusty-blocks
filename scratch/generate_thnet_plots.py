...
def generate_svg(csv_path, output_path, title, x_col, y1_cols, y2_cols=None, y1_labels=None, y2_labels=None, y1_unit="", y2_unit="", colors=None):
    ...
    if y2_cols:
        for i in range(n_ticks_y):
            ...
            y_pos = to_y2(val)
            svg.append(f'<text x="{m_left + plot_w + 10}" y="{y_pos + 4}" text-anchor="start" font-size="11" fill="#64748b">{val:.1f} {y2_unit}</text>')
    ...
    for s_idx, label in enumerate(y2_labels or y2_cols):
        ...
        svg.append(f'<text x="{offset_x + 22}" y="-4" font-size="11" fill="#334155">{label}</text>')
...
if __name__ == "__main__":
    ...
    # Plot Convección Natural Implícita (THNet)
    generate_svg(
        csv_path="results/conv_natural_thnet.csv",
        output_path=os.path.join(art_dir, "thnet_convection_plot.svg"),
        title="THNet: Convección Natural con HX (Tubo de 25cm, Alto 12m)",
        x_col="t_s",
        y1_cols=["W_kg_s"],
        y2_cols=["T_bot_C", "T_top_C"],
        y1_labels=["Caudal (kg/s)"],
        y2_labels=["Temp Header Inf (C)", "Temp Header Sup (C)"],
        y1_unit="kg/s",
        y2_unit="°C",
