...
fn main() {
    ...
    let ts = 0.02; // 20 ms time step (very stable with pressure damping)
    let alpha = 0.95; // 95% pressure filter damping factor

    // --- BLOQUES ---
    
    // Header Inferior (1.0 m3, seleccionamos T y P de nodo 0)
    let h_bottom = system.add_block(Box::new(DiscreteHeader::new(1.0, thermo.clone(), vec![0], vec![0], ts, alpha)));
    ...
    let h_top = system.add_block(Box::new(DiscreteHeader::new(1.0, thermo.clone(), vec![0], vec![0], ts, alpha)));
    ...
    let mut csv_file = File::create("discrete_natural_convection_results.csv").unwrap();
    ...
    println!("Iniciando simulación de convección natural en tiempo discreto...");
    ...
    for step_idx in 1..=steps {
        ...
        // DiscreteHeader has 3 states now
    ...
    println!("Simulación discreta de convección natural completada.");
}
